...
async function preprocessIcons(files) {
  ...
  for (let i = 0; i < files.length; i++) {
    ...
    try {
      ...
      if ((i + 1) % 20 === 0) {
        console.log(`  ✓ Preprocessed ${i + 1}/${files.length} icons...`);
      }
    } catch (e) {
      console.warn(`  ⚠️  Failed to preprocess ${file}:`, e.message);
    }
  }
  
  console.log(`✅ Preprocessed ${processedFiles.length} icons`);
...
async function main() {
  ...
  if (files.length === 0) {
    console.error('❌ No PNG files found in', ICONS_DIR);
    process.exit(1);
  }
  ...
  console.log(`📊 Creating sprite: ${spriteWidth}x${spriteHeight} (${totalIcons} icons in ${rows} rows)`);
  ...
  for (let i = 0; i < processedFiles.length; i++) {
    ...
    try {
      ...
      if ((i + 1) % 20 === 0) {
        console.log(`  ✓ Added ${i + 1}/${totalIcons} icons to sprite...`);
      }
    } catch (e) {
      console.warn(`  ⚠️  Failed to add ${file} to sprite:`, e.message);
    }
  }
  ...
  console.log(`✅ Sprite saved: ${OUTPUT_SPRITE} (${stats.width}x${stats.height})`);
...
`;
  ...
  console.log(`✅ Metadata saved: ${OUTPUT_METADATA}`);
  console.log(`\n📦 Sprite atlas complete: ${totalIcons} icons packed into ${spriteWidth}x${spriteHeight}px`);
}
